...
const logger = () => {
    return async function (ctx: Koa.Context, next: Koa.Next) {
        ...
        const log: ILog = {
            ...
            length: humanizeBytes(ctx.response.length.toString()),
        };
  
        const logStrs = [];
        for (const i in log) logStrs.push(log[i]);
        ...
        fs.stat(logPath, (err: Error, stats: fs.Stats) => {
...
export const humanizeBytes = (s: string) => {
    /**
   * 转换字节为 MB GB 等
   */
    let finalSize = '';
